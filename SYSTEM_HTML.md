...
| `typhoonid` | id, cname, sponsor, WARN1BEG/END, WARN2BEG/END | 颱風資料（安外欄位小寫 warnSeaBeg，安內大寫 WARN1BEG；用 AS 統一）；`sponsor='LocalTime'` 篩選需加 `COLLATE utf8mb4_general_ci`（上游庫定序較舊，混用會拋 Illegal mix of collations，且原本的 except 會把這個錯誤吞掉不顯示）|
